...
async def finish_dialog(msg: Message, state: FSMContext) -> None:
    ...
    logger.info("Finish command: user_id=%s", user_id)
    if _kafka_enabled() and user_id:
        send_event(TOPIC_DIALOG_LOGS, str(user_id), {
            "user_id": user_id,
...
async def force_diagnosis(msg: Message, state: FSMContext) -> None:
    user_id = msg.from_user.id if msg.from_user else None
    logger.info("Diagnosis command: user_id=%s", user_id)
...
async def handle_diagnosis(msg: Message, state: FSMContext) -> None:
    ...
    if _kafka_enabled() and user_id:
        send_event(TOPIC_DIAGNOSIS_RESULTS, str(user_id), {
            "user_id": user_id,
